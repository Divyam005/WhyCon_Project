...
void failsafeOnCrash(void)
{
    ...
    if(ARMING_FLAG(ARMED)) {
        if ( ABS(inclination.values.rollDeciDegrees) > 700 || ABS(inclination.values.pitchDeciDegrees)>700 || (ABS(accSmooth[0])>5000)|| (ABS(accSmooth[1])>5000) 
		#ifdef ENABLE_ACROBAT
		&&tstate<=1
		#endif
		) //to indicate that a crash has occurred// || (ABS(accSmooth[0])>5000)||(ABS(accSmooth[1])>5000)
        {
            flightIndicatorFlag = (1 << 6);
            mwDisarm();
