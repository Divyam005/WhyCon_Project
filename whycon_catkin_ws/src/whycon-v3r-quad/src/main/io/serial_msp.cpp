...
#define MSP_SET_POS              216    //in message         Send PosData
...
static bool processOutCommand(uint8_t cmdMSP)
{
    ...
    case MSP_STATUS:
		...
		//initialThrottleHold_tes t = initialThrottleHold_test + 1;
              //DRONA
	...
	case MSP_FLIGHT_STATUS:
        ...
        headSerialReply(2);
        serialize16(flightIndicatorFlag);
...
//        break;
    ...
    case MSP_ATTITUDE:
        ...
        for (i = 0; i < 2; i++) {
			...
			/* if(i==0)
				//serialize16((uint16_t)flightIndicatorFlag);
                //serialize16((uint16_t)altitudeHoldGetEstimatedAltitude());
				//serialize16(NewSensorRange*10);
            else
				//serialize16((uint16_t)masterConfig.magZero.raw[1]);
				//serialize16((int16_t)getSetVelocity()*10);
				//serialize16((uint16_t)Global_Status *10);
				//serialize16(debug_range*10); */
        }
        serialize16(heading);
		break;
    case MSP_ALTITUDE:
        headSerialReply(6);
           ...
           //serialize32((uint32_t)ToF_Height);
          //
