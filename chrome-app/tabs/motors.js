/**
 * Created by Neha on 22-10-2016.
 */
  var masterEnabled= [],
   
     arrayIndex;
 
 var checkedAttr = new Array(8).fill(0);
...
TABS.motors.initialize = function (callback) {
    ...
    function update_arm_status() {
        self.armed = bit_check(CONFIG.mode, 1);
    }
    ...
    function process_html() {
        ...
        $('#category-list :checkbox').prop('checked',false);
        $('#category-list :checkbox').prop('disabled', true);
        ...
        var masterSliderValue = $('div.sliders input.master').val();
        
        var uiTrue=1;
        
        var controlNonMasterState=0;
        
        //unlock valid checkbox
        $('#category-list :checkbox').slice(0, number_of_valid_outputs).prop('disabled', false);
        
        //unlock valid outputs
        $('div.tst input').slice(0, number_of_valid_outputs).prop('disabled', false);
        $('div.sliders input').slice(0, number_of_valid_outputs).prop('disabled', false);
        
        // unlock master text
         $('div.tst input:last').prop('disabled', false);
      ...
      console.log("checked indexes", $('#category-list :checkbox').index(this));
      arrayIndex = $('#category-list :checkbox').index(this);
      if(this.checked){
			checkedAttr[arrayIndex]=1;
		}
 		else
 			checkedAttr[arrayIndex]=0;
 		});
            ...
            // UI hooks
                ...
                buffering_set_motor1 = [],
                buffer_delay = false;
                var buffering_set_motor1 = [];
                
                //set non master input
            $('div.sliders input:not(.master)').on('input', function () {

               if(controlNonMasterState==1){
                controlNonMasterState=0;
                }
                else
               {
                   $('div.sliders input.master').val(1000);
                   $('div.tst input.master').val(1000);
               }
                ...
                //if checkbox active
                    ...
                    {
                        var val1=1000;
                        buffer1.push(lowByte(val1));
                        buffer1.push(highByte(val1));
                        buffering_set_motor1.push(buffer1);
                    }
                }
                if (!buffer_delay) {
                    buffer_delay = setTimeout(function () {
                        ...
                        {
                            buffer1 = buffering_set_motor1.pop();
                            MSP.send_message(MSP_codes.MSP_SET_MOTOR, buffer1);
                        }
                        buffering_set_motor = [];
                        buffer_delay = false;
                    }, 10);
        ...
        $('div.sliders input.master').on('input', function () {
            ...
            for (var i=0; i<number_of_valid_outputs;i++)
            {
          	...
          	if(checkedAttr[i]==1) {
          	  $('#n'+i).val(val);         
          	  $('#m'+i).val(val); 
          	  $('div.tst input:not(:last):first').trigger('input');      	
             }
              else {
            	$('#n'+i).val(MISC.mincommand);
            	$('#m'+i).val(MISC.mincommand);
            	$('div.slider input:not(:last):first').trigger('input'); 
            }
          	
            }                     
            controlNonMasterState=1;
           
        });
        
        for(var i=0; i< number_of_valid_outputs;i++){
        	
        if(checkedAttr[i]==0 && masterSliderValue) {
        ...
        $('#motorsEnableTestMode').change(function () {
            if ($(this).is(':checked')) {
                ...
                $('div.sliders input:not(.master)').trigger('input');
        ...
        $('div.tst input').prop('min', MISC.mincommand);
        $('div.tst input').prop('max', MISC.maxthrottle);
        $('div.values li:not(:last)').text(MISC.mincommand);
        if(self.feature3DEnabled && self.feature3DSupported) {
            //Arbitrary sanity checks
            //Note: values may need to be revisited
            if(_3D.neutral3d > 1575 || _3D.neutral3d < 1425)
                _3D.neutral3d = 1500;
            $('div.tst input').val(_3D.neutral3d);
        } else {
            $('div.tst input').val(MISC.mincommand);
        }
            // UI hooks
            var buffering_set_motor = [],
                buffer_delay = false;
            $('div.tst input:not(.master)').on('input', function () {
                var index = $(this).index(),
                    buffer = [],
                    i;
            //    $('div.values li').eq(index).text($(this).val());
                for (i = 0; i < 8; i++) {
                    var val;
                    if(self.allowTestMode) {
                        var val = parseInt($('div.tst input').eq(i).val());
                        if (val < 1000)
                            val = 1000;
                        else if (val >= 2000)
                            $(this).val(2000);
                        ...
                        $('div.sliders input').eq(i).val(val);
                    }
                    else
                    {
                       val=1000;
        ...
        $('div.tst input.master').on('input', function () {
        ...
        if (motors_running) {
            ...
            // motors are running adjust tst to current values
            var tst = $('div.tst input:not(.master)');
            var master_value = MOTOR_DATA[0];
            for (var i = 0; i < MOTOR_DATA.length; i++) {
                if (MOTOR_DATA[i] > 0) {
                    tst.eq(i).val(MOTOR_DATA[i]);
                    if (master_value != MOTOR_DATA[i]) {
                        master_value = false;
                    }
                }
            ...
            // slide master slider if condition is valid
            if (master_value) {
                $('div.tst input.master').val(master_value);
                
                $('div.tst input.master').trigger('input');
            }
        }
